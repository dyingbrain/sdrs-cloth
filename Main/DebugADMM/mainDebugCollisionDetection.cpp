#include <ADMM/CollisionDetector.h>
...
template <int N,bool CHECK_BF,int RES=32>
void debugCollision(int m,T eps,T rand,bool noNeighbor) {
  typedef std::chrono::high_resolution_clock Clock;
  //build mesh grid
  MeshExact mesh,obs;
  createGrid(RES,mesh);
  createGrid(RES,obs,rand);
  typedef CollisionDetector<N,N> Detector;
  typedef typename Detector::ID ID;
  std::cout << "DebugCollision brute-force=" << CHECK_BF;
  std::cout << " no-neighbor=" << noNeighbor;
  std::cout << " eps=" << eps << std::endl;
  Vec x0,x,x2;
  Detector detector(mesh,obs);
  ...
  for(int i=0; i<m; i++) {
    ...
    x2+=x0;
    {
      ...
      if(CHECK_BF) {
        auto beg=Clock::now();
        detector.generateBroadBF(x,x2,eps,eps/2,noNeighbor,true,true);
        std::cout << "BroadBF " << std::chrono::duration_cast<std::chrono::milliseconds>(Clock::now()-beg).count();
      } else {
        auto beg=Clock::now();
        detector.generateBroad(x,x2,eps,eps/2,noNeighbor,false,true,true);
        std::cout << "BroadS " << std::chrono::duration_cast<std::chrono::milliseconds>(Clock::now()-beg).count();
      }
      ...
      if(CHECK_BF) {
        ...
        auto beg=Clock::now();
        detector.generateBroad(x,x2,eps,eps/2,noNeighbor,true,true,true);
        std::cout << " BroadP " << std::chrono::duration_cast<std::chrono::milliseconds>(Clock::now()-beg).count() << std::endl;
      }
...
int main(int argc,char** argv) {
  debugCollision<3,true>(10,0.01f,0.1f,true);
  debugCollision<3,false>(10,0.01f,0.1f,true);
  debugCollision<2,true>(10,0.01f,0.1f,true);
  debugCollision<2,false>(10,0.01f,0.1f,true);
  return 0;
}
