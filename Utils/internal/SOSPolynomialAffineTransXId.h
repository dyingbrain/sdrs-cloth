...
struct AffineTransXId {
  static T transXId(const T& in,int coef,int off) {
    return in;
  }
};
template <typename T,char LABEL,char LABEL2>
struct AffineTransXId<SOSPolynomial<T,LABEL>,LABEL2> {
  static SOSPolynomial<T,LABEL> transXId(const SOSPolynomial<T,LABEL>& in,int coef,int off) {
    SOSPolynomial<T,LABEL> out=in;
    for(int i=0; i<(int)out._terms.size(); i++) {
      if(LABEL==LABEL2)
        for(int j=0; j<(int)out._terms[i]._id.size(); j++)
