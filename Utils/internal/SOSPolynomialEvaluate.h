...
class ParallelEvaluate {
 public:
  ...
  //utility
  static void gradient(VEC& g,int r,const T& v) {
    OMP_CRITICAL_ {
      T& val=g[r];
      val+=v;
    }
  }
  static void hessian(MAT& m,int r,int c,const T& v) {
    OMP_CRITICAL_ {
      T& val=m(r,c);
      val+=v;
    }
  }
  static void hessianSym(MAT& m,int r,int c,const T& v) {
    OMP_CRITICAL_ {
      T& val=m(r,c);
      val+=v;
    }
  ...
  static void hessian(STRIPS& m,int r,int c,const T& v) {
    m.push_back(STRIP(r,c,v));
  }
  static void hessianSym(STRIPS& m,int r,int c,const T& v) {
    m.push_back(STRIP(r,c,v));
  ...
  static T eval(POLY& p,const COLD& x,VEC* grad,HESS* hess) {
    T ret=0;
    for(int i=0; i<(int)p._terms.size(); i++)
      ret+=p._terms[i].eval(x,grad,hess);
    return ret;
  }
};
template <>
class ParallelEvaluate<double> {
 public:
  typedef double T;
  typedef typename Eigen::Matrix<T,-1,1> VEC;
  typedef typename Eigen::Matrix<T,-1,-1> MAT;
