...
template <typename T>
typename PBDArticulatedGradientInfoMap<T>::Mat3X4T PBDArticulatedGradientInfoMap<T>::DTDLambda(int k) const {
...
template <typename T>
void PBDArticulatedGradientInfoMap<T>::DTG(int k,const ArticulatedBody& body,Mat3X4T GK,std::function<void(int,T)> DTG) const {
  Eigen::Matrix<int,2,1> begEnd;
  Mat3T coefOmega;
...
template <typename T>
void PBDArticulatedGradientInfoMap<T>::toolA(int k,const ArticulatedBody& body,const PBDArticulatedGradientInfoMap& Tj,Mat3T MRR,Mat3T MRt,Mat3T MtR,Mat3T Mtt,std::function<void(int,int,T)> A) const {
...
template <typename T>
void PBDArticulatedGradientInfoMap<T>::toolA(const ArticulatedBody& body,const PBDArticulatedGradientInfoMap& Tj,Mat3XTM MRR,Mat3XTM MRt,Mat3XTM MtR,Mat3XTM Mtt,std::function<void(int,int,T)> A) const {
  Mat3T wK_1KiMwKj,wK_1KiMtKj;
  Mat3T tK_1KiMwKj,tK_1KiMtKj;
  Mat3T wK_1iMwK_1Kj,tK_1iMwK_1Kj;
  Mat3T wK_1iMtK_1Kj,tK_1iMtK_1Kj;
...
template <typename T>
void PBDArticulatedGradientInfoMap<T>::toolANonRecursivePhase1Rotational(int k,const ArticulatedBody& body,const PBDArticulatedGradientInfoMap& Tj,const Mat3T& wK_1KiMwKj,const Mat3T& wK_1KiMtKj,std::function<void(int,int,T)> A) const {
  Eigen::Matrix<int,2,1> begEnd;
  const Joint& J=body.joint(k);
  Tj.JRCSparse(body,k,[&](int col,const Vec3T& v) {
    for(begEnd=J.RBegEnd(); begEnd[0]<begEnd[1]; begEnd[0]++)
      A(begEnd[0],col,_DTM.col(begEnd[0]).dot(wK_1KiMwKj*v));
  },[&](int col,const Vec3T& v) {
    for(begEnd=J.RBegEnd(); begEnd[0]<begEnd[1]; begEnd[0]++)
      A(begEnd[0],col,_DTM.col(begEnd[0]).dot(wK_1KiMtKj*v));
  });
}
template <typename T>
void PBDArticulatedGradientInfoMap<T>::toolANonRecursivePhase1Translational(int k,const ArticulatedBody& body,const PBDArticulatedGradientInfoMap& Tj,const Mat3T& tK_1KiMwKj,const Mat3T& tK_1KiMtKj,std::function<void(int,int,T)> A) const {
  Eigen::Matrix<int,2,1> begEnd;
  const Joint& J=body.joint(k);
  Tj.JRCSparse(body,k,[&](int col,const Vec3T& v) {
    for(begEnd=J.CBegEnd(); begEnd[0]<begEnd[1]; begEnd[0]++)
...
template <typename T>
void PBDArticulatedGradientInfoMap<T>::toolANonRecursivePhase2Rotational(int k,const ArticulatedBody& body,const PBDArticulatedGradientInfoMap& Tj,const Mat3T& wK_1iMwK_1Kj,const Mat3T& tK_1iMwK_1Kj,std::function<void(int,int,T)> A) const {
  Eigen::Matrix<int,2,1> begEnd;
  const Joint& J=body.joint(k);
  JRCSparse(body,J._parent,[&](int col,const Vec3T& v) {
    for(begEnd=J.RBegEnd(); begEnd[0]<begEnd[1]; begEnd[0]++)
...
template <typename T>
void PBDArticulatedGradientInfoMap<T>::toolANonRecursivePhase2Translational(int k,const ArticulatedBody& body,const PBDArticulatedGradientInfoMap& Tj,const Mat3T& wK_1iMtK_1Kj,const Mat3T& tK_1iMtK_1Kj,std::function<void(int,int,T)> A) const {
  Eigen::Matrix<int,2,1> begEnd;
  const Joint& J=body.joint(k);
  JRCSparse(body,J._parent,[&](int col,const Vec3T& v) {
    for(begEnd=J.CBegEnd(); begEnd[0]<begEnd[1]; begEnd[0]++)
...
template <typename T>
void PBDArticulatedGradientInfoMap<T>::toolARecursive(int k,int p,const PBDArticulatedGradientInfoMap& Tj,Mat3XTM MRR,Mat3XTM MRt,Mat3XTM MtR,Mat3XTM Mtt) const {
...
template <typename T>
void PBDArticulatedGradientInfoMap<T>::toolARecursiveInplace(int k,int p,const PBDArticulatedGradientInfoMap& Tj,Mat3T& MRR,Mat3T& MRt,Mat3T& MtR,Mat3T& Mtt) const {
  Mat3T DC=cross<T>(CTRI(_TM,k)-CTRI(_TM,p));
  Mat3T DCj=cross<T>(CTRI(Tj._TM,k)-CTRI(Tj._TM,p));
...
void PBDArticulatedGradientInfoMap<T>::toolAZ(const ArticulatedBody& body,const PBDArticulatedGradientInfoMap& Tj,Mat3XTM MRR,Mat3XTM MRt,Mat3XTM MtR,Mat3XTM Mtt,MatTM A) const {
  A.setZero();
  toolA(body,Tj,MRR,MRt,MtR,Mtt,[&](int row,int col,T val) {
    A(row,col)+=val;
  });
}
...
void PBDArticulatedGradientInfoMap<T>::toolA(const ArticulatedBody& body,const PBDArticulatedGradientInfoMap& Tj,MatTCM M,MatTM A) const {
  ...
  toolA(body,Tj,mapM(MRR),mapM(MRt),mapM(MtR),mapM(Mtt),[&](int row,int col,T val) {
    A(row,col)+=val;
  });
}
template <typename T>
...
template <typename T>
void PBDArticulatedGradientInfoMap<T>::toolALR(int kL,int kR,const ArticulatedBody& body,const Mat3T& tensor,const Vec3T& ptL,const Vec3T& ptR,std::function<void(int,int,T)> A) const {
  Vec3T RPtL=ROTI(_TM,kL)*ptL,RPtR=ROTI(_TM,kR)*ptR,DRPtL;
  JRCSparse(body,kL,[&](int row,const Vec3T& JRL) {
    DRPtL=JRL.cross(RPtL);
    JRCSparse(body,kR,[&](int col,const Vec3T& JRR) {
...
template <typename T>
void PBDArticulatedGradientInfoMap<T>::toolALR(int kL,int kR,const ArticulatedBody& body,const PBDArticulatedGradientInfoMap& Tj,MatTCM tensor,MatTM A) const {
...
template <typename T>
void PBDArticulatedGradientInfoMap<T>::toolBNonRecursivePhase1Rotational(int k,const ArticulatedBody& body,const PBDArticulatedGradientInfoMap& Tj,const Mat3T& wK_1KiMwKj,std::function<void(int,int,T)> A) const {
  Eigen::Matrix<int,2,1> begEnd;
  const Joint& J=body.joint(k);
  Tj.JRSparse(body,k,[&](int col,const Vec3T& v) {
    for(begEnd=J.RBegEnd(); begEnd[0]<begEnd[1]; begEnd[0]++)
      A(begEnd[0],col,_DTM.col(begEnd[0]).dot(wK_1KiMwKj*v));
  });
}
template <typename T>
void PBDArticulatedGradientInfoMap<T>::toolBNonRecursivePhase1Translational(int k,const ArticulatedBody& body,const PBDArticulatedGradientInfoMap& Tj,const Mat3T& tK_1KiMwKj,std::function<void(int,int,T)> A) const {
  Eigen::Matrix<int,2,1> begEnd;
  const Joint& J=body.joint(k);
  Tj.JRSparse(body,k,[&](int col,const Vec3T& v) {
    for(begEnd=J.CBegEnd(); begEnd[0]<begEnd[1]; begEnd[0]++)
...
template <typename T>
void PBDArticulatedGradientInfoMap<T>::toolBNonRecursivePhase2Rotational(int k,const ArticulatedBody& body,const PBDArticulatedGradientInfoMap& Tj,const Mat3T& wK_1iMwK_1Kj,std::function<void(int,int,T)> A) const {
  Eigen::Matrix<int,2,1> begEnd;
  const Joint& J=body.joint(k);
  JRSparse(body,J._parent,[&](int col,const Vec3T& v) {
    for(begEnd=J.RBegEnd(); begEnd[0]<begEnd[1]; begEnd[0]++)
      A(col,begEnd[0],v.dot(wK_1iMwK_1Kj*Tj._DTM.col(begEnd[0])));
  });
}
template <typename T>
void PBDArticulatedGradientInfoMap<T>::toolBNonRecursivePhase2Translational(int k,const ArticulatedBody& body,const PBDArticulatedGradientInfoMap& Tj,const Mat3T& wK_1iMtK_1Kj,std::function<void(int,int,T)> A) const {
  Eigen::Matrix<int,2,1> begEnd;
  const Joint& J=body.joint(k);
  JRSparse(body,J._parent,[&](int col,const Vec3T& v) {
    for(begEnd=J.CBegEnd(); begEnd[0]<begEnd[1]; begEnd[0]++)
...
template <typename T>
void PBDArticulatedGradientInfoMap<T>::toolBRecursive(int k,int p,Mat3XTM G) const {
  GETTCM_T(JLT,_TK_1KM,k)
  GETTM_T(GK,G,k)
  GETTM_T(GKP,G,p)
...
template <typename T>
void PBDArticulatedGradientInfoMap<T>::toolBRecursiveInplace(int k,int p,Mat3X4T& G) const {
  GETTCM_T(JLT,_TK_1KM,k)
...
void PBDArticulatedGradientInfoMap<T>::toolBZ(const ArticulatedBody& body,Mat3XTM G,MatTM B) const {
  B.setZero();
  toolB(body,G,[&](int row,int col,T val) {
    B(row,col)+=val;
  });
}
//this is an interface whose input is a 12X12 tensor
...
#define CONTRACT(R0,R1,C0,C1) G(C0,C1+k*4)+=tensor(R0+R1*3,C0+C1*3)*TG1(R0,R1);
  ...
  toolB(body,mapM(G),[&](int row,int col,T val) {
    B(row,col)+=val;
  });
#undef CONTRACT
}
...
//this is an interface whose input is 4 3X3 tensor
template <typename T>
void PBDArticulatedGradientInfoMap<T>::toolAB(int k,const ArticulatedBody& body,Mat3T MRR,Mat3T MRt,Mat3T MtR,Mat3T Mtt,Mat3X4T G,std::function<void(int row,int col,T val)> AB) const {
  Mat3T wK_1KiMwKj,wK_1KiMtKj;
  Mat3T tK_1KiMwKj,tK_1KiMtKj;
  Mat3T wK_1iMwK_1Kj,tK_1iMwK_1Kj;
  Mat3T wK_1iMtK_1Kj,tK_1iMtK_1Kj;
...
template <typename T>
void PBDArticulatedGradientInfoMap<T>::toolAB(const ArticulatedBody& body,Mat3XTM MRR,Mat3XTM MRt,Mat3XTM MtR,Mat3XTM Mtt,Mat3XTM G,std::function<void(int row,int col,T val)> AB) const {
  Mat3T wK_1KiMwKj,wK_1KiMtKj;
  Mat3T tK_1KiMwKj,tK_1KiMtKj;
  Mat3T wK_1iMwK_1Kj,tK_1iMwK_1Kj;
  Mat3T wK_1iMtK_1Kj,tK_1iMtK_1Kj;
...
void PBDArticulatedGradientInfoMap<T>::toolABZ(const ArticulatedBody& body,Mat3XTM MRR,Mat3XTM MRt,Mat3XTM MtR,Mat3XTM Mtt,Mat3XTM G,MatTM AB) const {
  AB.setZero();
  toolAB(body,MRR,MRt,MtR,Mtt,G,[&](int row,int col,T val) {
    AB(row,col)+=val;
  });
}
//this is an interface whose input is a 12X12 tensor
...
void PBDArticulatedGradientInfoMap<T>::toolAB(const ArticulatedBody& body,MatTCM M,Mat3XTM G,MatTM AB) const {
  ...
  toolAB(body,mapM(MRR),mapM(MRt),mapM(MtR),mapM(Mtt),G,[&](int row,int col,T val) {
    AB(row,col)+=val;
  });
}
template <typename T>
...
template <typename T>
void PBDArticulatedGradientInfoMap<T>::toolACRecursive(int k,int p,const PBDArticulatedGradientInfoMap& Tj,Mat3XTM MRRA,Mat3XTM MRtA,Mat3XTM MtRA,Mat3XTM MttA,Mat3XTM MRRC,Mat3XTM MRtC,Mat3XTM MtRC,Mat3XTM MttC) const {
  Mat3T DC=cross<T>(CTRI(_TM,k)-CTRI(_TM,p));
  Mat3T DCj=cross<T>(CTRI(Tj._TM,k)-CTRI(Tj._TM,p));
...
void PBDArticulatedGradientInfoMap<T>::toolACZ(const ArticulatedBody& body,const PBDArticulatedGradientInfoMap& Tj,Mat3XTM MRRA,Mat3XTM MRtA,Mat3XTM MtRA,Mat3XTM MttA,Mat3XTM MRRC,Mat3XTM MRtC,Mat3XTM MtRC,Mat3XTM MttC,MatTM AC) const {
  AC.setZero();
  toolAC(body,Tj,MRRA,MRtA,MtRA,MttA,MRRC,MRtC,MtRC,MttC,[&](int row,int col,T val) {
    AC(row,col)+=val;
  });
}
//this is an interface whose input is a 12X12 tensor
...
void PBDArticulatedGradientInfoMap<T>::toolAC(const ArticulatedBody& body,const PBDArticulatedGradientInfoMap& Tj,MatTCM MA,MatTCM MC,MatTM AC) const {
  ...
  toolAC(body,Tj,mapM(MRRA),mapM(MRtA),mapM(MtRA),mapM(MttA),mapM(MRRC),mapM(MRtC),mapM(MtRC),mapM(MttC),[&](int row,int col,T val) {
    AC(row,col)+=val;
  });
}
template <typename T>
...
//this is for internal force's separate term
template <typename T>
void PBDArticulatedGradientInfoMap<T>::toolCLR(int kL,int kR,const ArticulatedBody& body,const Mat3T& tensor,const Vec3T& ptL,const Vec3T& ptR,std::function<void(int row,int col,T val)> C) const {
  Vec3T RPtL=ROTI(_TM,kL)*ptL;
  Vec3T CLRPtL=DWDLI(_DTLambdaM,kL).cross(RPtL);
  Vec3T RPtR=ROTI(_TM,kR)*ptR,DRPtL;
  JRCILambdaSparse(body,kL,[&](int row,const Vec3T& JRL) {
...
template <typename T>
void PBDArticulatedGradientInfoMap<T>::toolCLR(int kL,int kR,const ArticulatedBody& body,const PBDArticulatedGradientInfoMap& Tj,MatTCM tensor,MatTM A) const {
#define CONTRACT(R0,R1,C0,C1) val+=RT(R0,R1)*RTj(C0,C1)*tensor(R0+R1*3,C0+C1*3);
  int nrDOF=body.nrDOF();
  T val;
  Mat3X4T RT,RTj;
...
void PBDArticulatedGradientInfoMap<T>::toolDBF(const ArticulatedBody& body,Mat3XTM G,MatTM D) const {
  ...
  for(int k=nrJ-1; k>=0; k--) {
    ...
    RP=J._parent>=0? ROTI(_TM,J._parent): Mat3T(Mat3T::Identity());
    HRK_1KLambdaSparse(body,k,[&](int row,int col,const Mat3T& HR) {
    ...
    if(J.isRotational()) {
      ...
      toolBNonRecursivePhase1Rotational(k,body,*this,wK_1KiMwKj,[&](int row,int col,T val) {
        D(row,col)+=val;
      });
      toolBNonRecursivePhase2Rotational(k,body,*this,wK_1KiMwKj.transpose(),[&](int row,int col,T val) {
        D(row,col)+=val;
      });
      ...
      toolBNonRecursivePhase1Translational(k,body,*this,tK_1KiMwKj,[&](int row,int col,T val) {
        D(row,col)+=val;
      });
      toolBNonRecursivePhase2Translational(k,body,*this,tK_1KiMwKj.transpose(),[&](int row,int col,T val) {
        D(row,col)+=val;
      });
    }
    if(J._parent>=0) {
      //term3: second order derivatives of T_{K-1}^K
      HRK_1KSparse(body,k,[&](int row,int col,const Mat3T& HR) {
...
template <typename T>
void PBDArticulatedGradientInfoMap<T>::toolDRecursive(int k,int p,Mat3XTM G,Mat3XTM GB,T coef) const {
...
void PBDArticulatedGradientInfoMap<T>::toolABCCTDZ(const ArticulatedBody& body,Mat3XTM MRRA,Mat3XTM MRtA,Mat3XTM MtRA,Mat3XTM MttA,Mat3XTM MRRC,Mat3XTM MRtC,Mat3XTM MtRC,Mat3XTM MttC,Mat3XTM G,Mat3XTM GB,MatTM ABCCTD) const {
  ABCCTD.setZero();
  toolABCCTD(body,MRRA,MRtA,MtRA,MttA,MRRC,MRtC,MtRC,MttC,G,GB,[&](int row,int col,T val) {
    ABCCTD(row,col)+=val;
    ABCCTD(col,row)+=val;
  });
}
...
void PBDArticulatedGradientInfoMap<T>::toolABCCTD(const ArticulatedBody& body,MatTCM MA,MatTCM MC,Mat3XTM G,MatTM ABCCTD) const {
  ...
  toolABCCTD(body,mapM(MRRA),mapM(MRtA),mapM(MtRA),mapM(MttA),mapM(MRRC),mapM(MRtC),mapM(MtRC),mapM(MttC),G,mapM(GB),[&](int row,int col,T val) {
    ABCCTD(row,col)+=val;
    ABCCTD(col,row)+=val;
  });
}
...
template <typename T>
void PBDArticulatedGradientInfoMap<T>::JRMSparse(const ArticulatedBody& body,int JID,std::function<void(int,const Mat3T&)> JR) const {
  JRSparse(body,JID,[&](int c,const Vec3T& R) {
...
template <typename T>
void PBDArticulatedGradientInfoMap<T>::JRSparse(const ArticulatedBody& body,int JID,VecM dx,Mat3T& dR) const {
  dR.setZero();
  JRMSparse(body,JID,[&](int c,const Mat3T& R) {
    dR+=dx[c]*R;
  });
}
template <typename T>
void PBDArticulatedGradientInfoMap<T>::JCSparse(const ArticulatedBody& body,int JID,VecM dx,Vec3T& dC) const {
  dC.setZero();
  JCSparse(body,JID,[&](int c,const Vec3T& v) {
    dC+=dx[c]*v;
  });
}
template <typename T>
typename PBDArticulatedGradientInfoMap<T>::Mat3T PBDArticulatedGradientInfoMap<T>::JRSparse(const ArticulatedBody& body,int JID,VecM dx) const {
  Mat3T dR;
  JRSparse(body,JID,dx,dR);
  return dR;
}
template <typename T>
typename PBDArticulatedGradientInfoMap<T>::Vec3T PBDArticulatedGradientInfoMap<T>::JCSparse(const ArticulatedBody& body,int JID,VecM dx) const {
  Vec3T dC;
  JCSparse(body,JID,dx,dC);
  return dC;
}
template <typename T>
void PBDArticulatedGradientInfoMap<T>::JRSparse(const ArticulatedBody& body,int JID,Mat3XTM dR) const {
  dR.setZero();
  JRSparse(body,JID,[&](int c,const Vec3T& v) {
    dR.col(c)+=v;
  });
}
template <typename T>
void PBDArticulatedGradientInfoMap<T>::JCSparse(const ArticulatedBody& body,int JID,Mat3XTM dC) const {
  dC.setZero();
  JCSparse(body,JID,[&](int c,const Vec3T& v) {
    dC.col(c)+=v;
  });
...
template <typename T>
void PBDArticulatedGradientInfoMap<T>::JRMILambdaSparse(const ArticulatedBody& body,int JID,std::function<void(int,const Mat3T&)> JR) const {
  JRILambdaSparse(body,JID,[&](int c,const Vec3T& R) {
...
template <typename T>
void PBDArticulatedGradientInfoMap<T>::JRILambdaSparse(const ArticulatedBody& body,int JID,VecM dx,Mat3T& dR) const {
  dR.setZero();
  JRMILambdaSparse(body,JID,[&](int c,const Mat3T& R) {
    dR+=dx[c]*R;
  });
}
template <typename T>
void PBDArticulatedGradientInfoMap<T>::JCILambdaSparse(const ArticulatedBody& body,int JID,VecM dx,Vec3T& dC) const {
  dC.setZero();
  JCILambdaSparse(body,JID,[&](int c,const Vec3T& v) {
    dC+=dx[c]*v;
  });
}
template <typename T>
typename PBDArticulatedGradientInfoMap<T>::Mat3T PBDArticulatedGradientInfoMap<T>::JRILambdaSparse(const ArticulatedBody& body,int JID,VecM dx) const {
  Mat3T dR;
  JRILambdaSparse(body,JID,dx,dR);
  return dR;
}
template <typename T>
typename PBDArticulatedGradientInfoMap<T>::Vec3T PBDArticulatedGradientInfoMap<T>::JCILambdaSparse(const ArticulatedBody& body,int JID,VecM dx) const {
  Vec3T dC;
  JCILambdaSparse(body,JID,dx,dC);
  return dC;
}
template <typename T>
void PBDArticulatedGradientInfoMap<T>::JRILambdaSparse(const ArticulatedBody& body,int JID,Mat3XTM dR) const {
  dR.setZero();
  JRILambdaSparse(body,JID,[&](int c,const Vec3T& v) {
    dR.col(c)+=v;
  });
}
template <typename T>
void PBDArticulatedGradientInfoMap<T>::JCILambdaSparse(const ArticulatedBody& body,int JID,Mat3XTM dC) const {
  dC.setZero();
  JCILambdaSparse(body,JID,[&](int c,const Vec3T& v) {
    dC.col(c)+=v;
  });
...
//-------------------------------------------------------------Hessian RK_1K
template <typename T>
void PBDArticulatedGradientInfoMap<T>::HRK_1KSparse(const ArticulatedBody& body,int JID,std::function<void(int,int,const Mat3T&)> HR) const {
  Mat3T DRIJ;
...
void PBDArticulatedGradientInfo<T>::reorthogonalize(const ArticulatedBody& body) {
  ...
  if(_JTrans.rows()!=3 || _JTrans.cols()!=nrJ*4) {
    ...
    for(int j=0; j<nrJ; j++) {
      ...
      JTrans.col(1)/=JTrans.col(1).norm();
...
void PBDArticulatedGradientInfo<T>::debug(const ArticulatedBody& body) {
  ...
  for(int JID=0; JID<nrJ; JID++) {
    ...
    //debug JR,JC
    DEBUG_GRADIENT("dR",G.JRSparse(body,JID,mapV(dx)).norm(),(G.JRSparse(body,JID,mapV(dx))-(ROTI(G2._TM,JID)-ROTI(G._TM,JID))/DELTA).norm())
    DEBUG_GRADIENT("dC",G.JCSparse(body,JID,mapV(dx)).norm(),(G.JCSparse(body,JID,mapV(dx))-(CTRI(G2._TM,JID)-CTRI(G._TM,JID))/DELTA).norm())
    //debug JRILambda,JCILambda
    G.resetLambda(body,lambda);
    DEBUG_GRADIENT("dRILambda",G.JRILambdaSparse(body,JID,mapV(dx)).norm(),(G.JRILambdaSparse(body,JID,mapV(dx))-(G4.JRSparse(body,JID,mapV(dx))-G.JRSparse(body,JID,mapV(dx)))/DELTA).norm())
    DEBUG_GRADIENT("dCILambda",G.JCILambdaSparse(body,JID,mapV(dx)).norm(),(G.JCILambdaSparse(body,JID,mapV(dx))-(G4.JCSparse(body,JID,mapV(dx))-G.JCSparse(body,JID,mapV(dx)))/DELTA).norm())
    //debug HR
    G.HRK_1KSparse(body,JID,[&](int row,int col,const Mat3T& HR) {
      ...
      DEBUG_GRADIENT("HR",HR.norm(),(HR-(DR2-DR)/DELTA).norm())
      row-=body.joint(JID)._offDOF;
      col-=body.joint(JID)._offDOF;
      HRss[row][col]=HR;
    });
    G4.HRK_1KSparse(body,JID,[&](int row,int col,const Mat3T& HR) {
      row-=body.joint(JID)._offDOF;
      col-=body.joint(JID)._offDOF;
      HRss2[row][col]=HR;
    });
    G.HRK_1KLambdaSparse(body,JID,[&](int row,int col,const Mat3T HRLambda) {
      row-=body.joint(JID)._offDOF;
      col-=body.joint(JID)._offDOF;
      DEBUG_GRADIENT("HRLambda",HRLambda.norm(),(HRLambda-(HRss2[row][col]-HRss[row][col])/DELTA).norm())
    });
    //debug DTG
    ...
    DEBUG_GRADIENT("toolABCCTD",ABCCTD.norm(),(ABCCTD-ABCCTDBF).norm())
  }
}
template <typename T>
...
template struct PBDArticulatedGradientInfoMap<FLOAT>;
template struct PBDArticulatedGradientInfo<FLOAT>;
