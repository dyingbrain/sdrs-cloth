...
template <typename T>
void JointFunc<T>::DDT(const Joint& J,std::function<void(int,int,T)> hess,Mat3XTCM DDT,const Vec3T& coefRss) {
...
void JointFunc<T>::DDT(const Joint& J,MatTM hess,Mat3XTCM DDT,const Vec3T& coefRss) {
  JointFunc<T>::DDT(J,[&](int row,int col,T val) {
    hess(row,col)+=val;
  },DDT,coefRss);
}
template <typename T>
typename JointFunc<T>::Vec3T JointFunc<T>::DDT(const Joint& J,Mat3XTCM DDT,int R,int C) {
  if(J._typeJoint == Joint::ROT_3D_EXP || J._typeJoint == Joint::ROT_3D_XYZ) {
...
template struct JointFunc<FLOAT>;
