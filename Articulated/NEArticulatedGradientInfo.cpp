...
//-------------------------------------------------------------RNEADerivatives
template <typename T>
void NEArticulatedGradientInfoMap<T>::DvDdq(const ArticulatedBody& body,int i) {
  const Joint& J=body.joint(i);
  int nrDOF=J.nrDOF();
  _DvDdqM.block(i*6,J._offDOF,6,nrDOF)+=_SM.block(0,J._offDOF,6,nrDOF);
  for(int j=body.joint(i)._parent,p=j; j>=0; j=body.joint(j)._parent) {
...
template <typename T>
void NEArticulatedGradientInfoMap<T>::DvDq(const ArticulatedBody& body,int i) {
  const Joint& J=body.joint(i);
  int nrDOF=J.nrDOF();
  for(int d=0; d<nrDOF; d++)
...
template <typename T>
void NEArticulatedGradientInfoMap<T>::DaDdq(const ArticulatedBody& body,int i,Vec6TCM vJ) {
  const Joint& J=body.joint(i);
  int nrDOF=J.nrDOF();
  for(int d=0; d<nrDOF; d++)
  ...
  for(int j=i; j>=0; j=body.joint(j)._parent) {
    const Joint& JP=body.joint(j);
    int nrDOFP=JP.nrDOF();
    for(int d=0; d<nrDOFP; d++)
      _DaDdqM.block(i*6,JP._offDOF+d,6,1)+=spatialCross<T>(_DvDdqM.block(i*6,JP._offDOF+d,6,1),vJ);
  }
}
template <typename T>
void NEArticulatedGradientInfoMap<T>::DaDq(const ArticulatedBody& body,int i,Vec6TCM vJ) {
  const Joint& J=body.joint(i);
  int nrDOF=J.nrDOF();
...
template <typename T>
void NEArticulatedGradientInfoMap<T>::Dfx(const ArticulatedBody& body,int i,Vec6TCM fx) {
...
template <typename T>
void NEArticulatedGradientInfoMap<T>::Df(const ArticulatedBody& body,int i,Vec6TCM vJ,Mat6XTCM IMCustom) {
  if(!vJ.data()) {
    for(int j=i; j>=0; j=body.joint(j)._parent) {
      const Joint& JP=body.joint(j);
      int nrDOFP=JP.nrDOF();
...
template <typename T>
void NEArticulatedGradientInfoMap<T>::Dtau(const ArticulatedBody& body,int i,int j) {
  const Joint& J=body.joint(i);
...
template <typename T>
void NEArticulatedGradientInfoMap<T>::DHDq(const ArticulatedBody& body,int i,int j,MatTM dHdq) const {
  const Joint& J=body.joint(i);
  const Joint& JJ=body.joint(j);
  int nrDOF=J.nrDOF();
  int nrDOFJ=JJ.nrDOF();
...
template <typename T>
void NEArticulatedGradientInfoMap<T>::DfP(const ArticulatedBody& body,int i,int j,bool dq) {
  int p=body.joint(i)._parent;
...
//-------------------------------------------------------------MDP
template <typename T>
void NEArticulatedGradientInfoMap<T>::Sc(const ArticulatedBody& body,Eigen::Map<const Eigen::Matrix<int,-1,1>> joints,MatTM sc) const {
  sc.setZero();
...
template <typename T>
void NEArticulatedGradientInfoMap<T>::DScDqT(const ArticulatedBody& body,Eigen::Map<const Eigen::Matrix<int,-1,1>> joints,MatTM DscDqTr,VecCM r) {
  _vM.setZero();
  _nonZeroM.setZero();
  for(int i=0; i<(int)joints.size(); i++) {
    int ip=joints[i];
...
template <typename T>
void NEArticulatedGradientInfoMap<T>::DScDq(const ArticulatedBody& body,Eigen::Map<const Eigen::Matrix<int,-1,1>> joints,MatTM DscDqr,VecCM r) const {
  Mat6T TJ;
  Vec6T Scr;
  Mat6X3T DSDqf;
  DscDqr.setZero();
...
template <typename T>
void NEArticulatedGradientInfoMap<T>::ScInvH(const ArticulatedBody& body,Eigen::Map<const Eigen::Matrix<int,-1,1>> joints,MatTM sc,MatTM scInvH) const {
  scInvH.setZero();
  Sc(body,joints,sc);
  for(int i=0; i<(int)joints.size(); i++) {
    int ip=joints[i];
...
template <typename T>
void NEArticulatedGradientInfoMap<T>::ScInvHScT(const ArticulatedBody& body,Eigen::Map<const Eigen::Matrix<int,-1,1>> joints,MatTM sc,MatTM scInvH,MatTM scInvHscT) const {
  scInvHscT.setZero();
...
template <typename T>
void NEArticulatedGradientInfoMap<T>::DTG(int k,const ArticulatedBody& body,Mat3X4T GK,std::function<void(int,T)> dtg) const {
  int ip=k;
...
template <typename T>
void NEArticulatedGradientInfoMap<T>::DTG(int k,const ArticulatedBody& body,Mat3X4T GK,VecM dtg) const {
  DTG(k,body,GK,[&](int off,T val) {
    dtg[off]+=val;
  });
}
template <typename T>
...
template <typename T>
typename NEArticulatedGradientInfoMap<T>::Mat3X4T NEArticulatedGradientInfoMap<T>::getTrans(int JID) const {
...
void NEArticulatedGradientInfoMap<T>::calcHInner(const ArticulatedBody& body,MatTM H,Mat6XTCM IMCustom) {
  ...
  for(int i=body.nrJ()-1; i>=0; i--) {
    ...
    Eigen::Block<Mat6XTM> Si=_SM.block(0,J._offDOF,6,nrDOF);
    Eigen::Block<Mat6X3T> Fb=F.block(0,0,6,nrDOF);
    Fb=TRANSI6(_ICM,i)*Si;
    H.block(J._offDOF,J._offDOF,nrDOF,nrDOF)=Si.transpose()*Fb;
    for(int j=i; body.joint(j)._parent>=0;) {
      Fb=(TRANSI6(_invTPM,j).transpose()*Fb).eval();
      j=body.joint(j)._parent;
      const Joint& Jj=body.joint(j);
      int nrDOFj=Jj.nrDOF();
      Eigen::Block<Mat6XTM> Sj=_SM.block(0,Jj._offDOF,6,nrDOFj);
      H.block(J._offDOF,Jj._offDOF,nrDOF,nrDOFj)=Fb.transpose()*Sj;
      if(nrDOF>0 && nrDOFj>0)
        H.block(Jj._offDOF,J._offDOF,nrDOFj,nrDOF)=H.block(J._offDOF,Jj._offDOF,nrDOF,nrDOFj).transpose();
    }
  }
}
template <typename T>
...
void NEArticulatedGradientInfoMap<T>::LTDL() {
  //factorize
  T a;
  for(int k=(int)_invHM.rows()-1,i,j; k>=0; k--) {
    i=_sparsityM[k];
    while(i>=0) {
...
void NEArticulatedGradientInfoMap<T>::ABAInner(const ArticulatedBody& body,Vec6TCM a0,Mat6XTCM fx,VecCM tau0,Mat6XTCM IMCustom,MatTCM diag) {
  ...
  for(int i=body.nrJ()-1; i>=0; i--) {
    ...
    int nrDOF=J.nrDOF();
    Eigen::Block<Mat6XTM> Si=_SM.block(0,J._offDOF,6,nrDOF);
    Eigen::Block<Mat6XTM> Ui=_UM.block(0,J._offDOF,6,nrDOF);
  ...
  for(int i=0; i<body.nrJ(); i++) {
    ...
    Eigen::Block<MatTM> Di=_HM.block(J._offDOF,J._offDOF,nrDOF,nrDOF);
    Eigen::Block<VecM,-1,1> taui=_tauM.segment(J._offDOF,nrDOF);
    _ddqM.segment(J._offDOF,nrDOF)=Di*(taui-Ui.transpose()*aPrime);
    _aM.col(i)=aPrime+Si*_ddqM.segment(J._offDOF,nrDOF);
  }
}
template <typename T>
void NEArticulatedGradientInfoMap<T>::ABA(const ArticulatedBody& body,Vec6TCM a0,Mat6XTCM fx,VecCM tau0) {
  ABAInner(body,a0,fx,tau0);
}
template <typename T>
...
void NEArticulatedGradientInfo<T>::reorthogonalize(const ArticulatedBody& body) {
  ...
  for(int i=body.nrJ()-1; i>=0; i--) {
    const Joint& J=body.joint(i);
    for(int d=J._offDOF+J.nrDOF()-1; d>J._offDOF; d--)
      _sparsity[d]=d-1;
...
void NEArticulatedGradientInfo<T>::debug(const ArticulatedBody& body) {
  ...
  for(int JID=0; JID<nrJ; JID++) {
    std::cout << "-------------------------------------------------------------DebugNEArticulatedGradientInfo" << std::endl;
    //calculate info
    q.setRandom();
    dq.setRandom();
    ddq.setRandom();
    ...
    GPBD.reset(body,q);
    DEBUG_GRADIENT("T",TRANSI6(G._invTM,JID).norm(),(spatialInv<T>(TRANSI6(G._invTM,JID))-toSpatial<T>(TRANSI(GPBD._TM,JID))).norm())
    ...
    DEBUG_GRADIENT("v",v.norm(),(v-v2).norm())

    a=G._aM.col(JID);
    a2=(G2._vM.col(JID)-G._vM.col(JID))/DELTA;
    DEBUG_GRADIENT("a",q.norm(),(a-a2).norm())
    ...
    DEBUG_GRADIENT("g",G._tauM.norm(),(G._tauM-G2._tauM).norm())
    a0.setRandom();
    fx.setRandom();
    ...
    DEBUG_GRADIENT("CRBA-LTL",tau0.norm(),(G._tau-tau0).norm())

    G.reset(body,q,dq);
    G.CRBA(body,mapCV<Vec6T>(a0),mapCM<Mat6XT>(fx),mapCV(tau0),true);
    G.RNEA(body,mapCV<Vec6T>(a0),mapCM<Mat6XT>(fx));
    DEBUG_GRADIENT("CRBA-LTDL",tau0.norm(),(G._tau-tau0).norm())

    G.reset(body,q,dq);
    G.ABA(body,mapCV<Vec6T>(a0),mapCM<Mat6XT>(fx),mapCV(tau0));
    G.RNEA(body,mapCV<Vec6T>(a0),mapCM<Mat6XT>(fx));
    DEBUG_GRADIENT("ABA",tau0.norm(),(G._tau-tau0).norm())
    ...
    DEBUG_GRADIENT("ABADerivatives-q",(G.getDddqDq()*deltaq).norm(),(G.getDddqDq()*deltaq-(G2._ddq-G._ddq)/DELTA_MULT).norm())
    ...
    G2.ABA(body,mapCV<Vec6T>(a0),mapCM<Mat6XT>(fx),mapCV(tau0));
    DEBUG_GRADIENT("ABADerivatives-dq",(G.getDddqDdq()*deltaq).norm(),(G.getDddqDdq()*deltaq-(G2._ddq-G._ddq)/DELTA_MULT).norm())
    ...
    DEBUG_GRADIENT("DvDq",(G._DvDqM.block(JID*6,0,6,nrDOF)*deltaq).norm(),(G._DvDqM.block(JID*6,0,6,nrDOF)*deltaq-(G2._vM.col(JID)-G._vM.col(JID))/DELTA).norm());
    DEBUG_GRADIENT("DaDq",(G._DaDqM.block(JID*6,0,6,nrDOF)*deltaq).norm(),(G._DaDqM.block(JID*6,0,6,nrDOF)*deltaq-(G2._aM.col(JID)-G._aM.col(JID))/DELTA).norm());
    DEBUG_GRADIENT("DfDq",(G._DfDqM.block(JID*6,0,6,nrDOF)*deltaq).norm(),(G._DfDqM.block(JID*6,0,6,nrDOF)*deltaq-(G2._fM.col(JID)-G._fM.col(JID))/DELTA).norm());
    DEBUG_GRADIENT("DtauDq",(G._DtauDqM*deltaq).norm(),(G._DtauDqM*deltaq-(G2._tauM-G._tauM)/DELTA).norm());
    G2.reset(body,q,dq+deltaq*DELTA,ddq);
    G2.RNEA(body,mapCV<Vec6T>(a0),mapCM<Mat6XT>(fx),true);
    DEBUG_GRADIENT("DvDdq",(G._DvDdqM.block(JID*6,0,6,nrDOF)*deltaq).norm(),(G._DvDdqM.block(JID*6,0,6,nrDOF)*deltaq-(G2._vM.col(JID)-G._vM.col(JID))/DELTA).norm());
    DEBUG_GRADIENT("DaDdq",(G._DaDdqM.block(JID*6,0,6,nrDOF)*deltaq).norm(),(G._DaDdqM.block(JID*6,0,6,nrDOF)*deltaq-(G2._aM.col(JID)-G._aM.col(JID))/DELTA).norm());
    DEBUG_GRADIENT("DfDdq",(G._DfDdqM.block(JID*6,0,6,nrDOF)*deltaq).norm(),(G._DfDdqM.block(JID*6,0,6,nrDOF)*deltaq-(G2._fM.col(JID)-G._fM.col(JID))/DELTA).norm());
    DEBUG_GRADIENT("DtauDdq",(G._DtauDdqM*deltaq).norm(),(G._DtauDdqM*deltaq-(G2._tauM-G._tauM)/DELTA).norm());
    ...
    Eigen::Map<const Eigen::Matrix<int,-1,1>> jointsM(&joints[0],(int)joints.size());
    std::cout << "Testing random joints: " << jointsM.transpose() << std::endl;
    ...
    DEBUG_GRADIENT("Sc-tau",(-Sc.transpose()*dfx).norm(),(-Sc.transpose()*dfx-(G2._tau-G._tau)/DELTA).norm())
    G.calcHInvH(body);
    ...
    DEBUG_GRADIENT("ScInvH",ScInvH.norm(),(ScInvH-Sc*G._invH).norm())
    DEBUG_GRADIENT("ScInvHScT",ScInvHScT.norm(),(ScInvHScT-Sc*G._invH*Sc.transpose()).norm())
    DEBUG_GRADIENT("ScInvH-ddq",(ScInvH.transpose()*dfx).norm(),(ScInvH.transpose()*dfx-(G2._ddq-G._ddq)/DELTA).norm())
    ...
    DEBUG_GRADIENT("DHDq",(DHDqr*deltaq).norm(),(DHDqr*deltaq-(G2._HM-G._HM)*r/DELTA).norm())
    DEBUG_GRADIENT("DScDqr",(DScDqr*deltaq).norm(),(DScDqr*deltaq-(Sc2-Sc)*r/DELTA).norm())
    DEBUG_GRADIENT("DScDqTr",(DScDqTr*deltaq).norm(),(DScDqTr*deltaq-(Sc2-Sc).transpose()*dfx/DELTA).norm())
  }
}
template <typename T>
void NEArticulatedGradientInfo<T>::resetPtr() {
  new (&_sparsityM)    Eigen::Map<Eigen::Matrix<int,-1,1>>(mapV(_sparsity));
  new (&_nonZeroM)     Eigen::Map<Eigen::Matrix<int,-1,1>>(mapV(_nonZero));
...
template struct NEArticulatedGradientInfoMap<FLOAT>;
template struct NEArticulatedGradientInfo<FLOAT>;
